...
export async function GET(request: Request) {
  ...
  const videoEndpoint = `${process.env.API_BASE}/search?part=snippet&maxResults=10&q=${query}&type=video&regionCode=PH&key=${process.env.YOUTUBE_API_1}`;
  const videoResponse = await fetch(videoEndpoint);
  const videoData = await videoResponse.json();
  const playlistEndpoint = `${process.env.API_BASE}/search?part=snippet&maxResults=5&q=${query}&type=playlist&regionCode=PH&key=${process.env.YOUTUBE_API_1}`;
